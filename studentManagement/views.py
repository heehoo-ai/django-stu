...
from .forms import StudentInformationModelForm
    ...
    if request.method == "POST":
        ...
        if id == root_id:
            ...
            if not all([stu_id, stu_name]):
                ...
                return render(request, 'studentManage/add.html', context)
            form = StudentInformationModelForm(request.POST)
            if form.is_valid() and form.clean_phone():
                form.save()
                student = StudentInformationModel.objects.get(stu_id=stu_id) #stu_id 重复会报错
                context = {
                    'students': student,
                    'form': form,
                    'sucess': '增加成功',
                }
                return render(request, 'studentManage/add.html', context=context)
        else:
            context = {
                'error': '只用root用户和管理员才能操作'
            }
...
def select(request):
    if request.method == "POST":
        ...
        # filter方法是从数据库的取得匹配的结果，返回一个模型对象列表
...
def update(request):
    ...
    if request.method == "POST":
        ...
        if id == stu_id:
            ...
            return render(request, 'studentManage/update.html', {'msg': '无权限修改他人信息'})
    else:
        return render(request, 'studentManage/update.html', context)
